...
<body>
  ...
  <div class="container-fluid layout" id="wrapper">
    <nav class="navbar navbar-default navbar-expand-md navbar-light navbar-laravel" role="navigation">
      <div class="container-fluid container">
        <div class="navbar-header">
          <a class="navbar-brand" href="{{ url('/RBAC/administrator/index') }}">
        ...
        <ul class="nav navbar-nav navbar-right">
          <li class="dropdown">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
              {{ session('lang') == 'en' ? 'English' : '中文' }} <b class="caret"></b>
            </a>
            <ul style="min-width: 100px;"  class="dropdown-menu">
              <li><a class="dropdown-item" href="{{ route('lang', ['locale' => 'en']) }}">
                {{ __('English') }}
              </a></li>
              <li><a class="dropdown-item" href="{{ route('lang', ['locale' => 'zh-CN']) }}">
                {{ __('中文') }}
              </a></li>
            </ul>
          </li>
    ...
    Dropzone.options.uploadForm = {
      ...
      init: function() {
        ...
        this.on('success', function(file, response) {
          if (typeof response == 'string' && response.replace(/[\r\n]/g,"") == 'OK') {
      ...
      acceptedFiles: "{{ config('lfm.should_validate_mime') ? lcfirst(str_singular(request('type') ?: '')) == 'image' ? implode(',', config('lfm.valid_image_mimetypes')) : implode(',', config('lfm.valid_file_mimetypes')) : ''}}",
