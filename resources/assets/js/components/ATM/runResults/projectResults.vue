<template>
  <div>
    <project-tool-bar :name="false">
      ...
      <div slot="operation">
        <el-button class="button_text_table el_button_result" @click="NavigationDebugTestCase">{{ lang.operator.debug_result }}</el-button>
      </div>
    </project-tool-bar>
  ...
  export default {
    ...
    methods: {
      ...
      NavigationToTestCasesResults(row) {
        window.location.href = '/atm/RunResult/Project/' + row.projectId + '/TestCase?page=1+25';
      },
      NavigationDebugTestCase() {
        window.location.href = '/atm/DebugResult/Project?page=1+25';
      },
      getMessageDetails() {
        const obj = {};
        for (var i in this.queryObj) {
          if (this.queryObj[i] != '') {
            if (i == 'ids') {
              obj.projectIds = this.queryObj[i];
            } else if (i == 'name') {
              obj.projectName = '%' + this.queryObj[i] + '%';
            } else if (i == 'startDate') {
              obj.projectStartDate = this.queryObj[i];
            } else if (i == 'endDate') {
              obj.projectEndDate = this.queryObj[i];
            } else {
