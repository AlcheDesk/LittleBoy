...
use App\Mappers\ATM\ApiSchemaMapper;
use App\Services\Service;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class ApiSchemaService implements Service
{
    ...
    {
        $this->mapper = new ApiSchemaMapper();
    }
    ...
    public function getProjectByApiSchemaId(Request $request, $apiSchemaId) : string
    {
        $jsonStringResponse = $this->mapper->getProjectByApiSchemaId($this->mapper->parse_query($request->getQueryString()), $apiSchemaId);
        Log::debug('Service layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return $jsonStringResponse;
    }
}
