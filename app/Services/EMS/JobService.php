...
use App\Mappers\EMS\JobMapper;
use App\Services\UuidService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class JobService implements UuidService {
	...
	// 关联接口
	public function getTaskByJobUuidRequest(Request $request, $jobUuid): string{
		$jsonStringResponse = $this->mapper->getTaskByJobUuid($this->mapper->parse_query($request->getQueryString()), $jobUuid);
		Log::debug('Service layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
		return $jsonStringResponse;
	}
}
