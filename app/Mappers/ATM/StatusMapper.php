...
class StatusMapper extends ATMBaseMapper implements BaseMapper
{
    ...
    public function countByOptions(array $options = [], $concurrentRequest = false): string
    {
        $options[] = 'count';
        // generate the url
        $url = $this->generateUrl("/api/statuses", $this->build_query($options));
        // return as request
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/statuses/" . $id);
        if ($concurrentRequest) {
            return new Request('DELETE', $url);
        }
        // sent to the data source server
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/statuses");
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/statuses/" . $id, $this->build_query($options));
        // return as request
        if ($concurrentRequest) {
            return new Request('GET', $url);
        }
