...
class PropertyMapper extends ATMBaseMapper implements BaseMapper
{
    ...
    public function countByOptions(array $options = [], $concurrentRequest = false): string
    {
        $options[] = 'count';
        // generate the url
        $url = $this->generateUrl("/api/properties", $this->build_query($options));
        // return as request
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/properties/" . $id);
        if ($concurrentRequest) {
            return new Request('DELETE', $url);
        }
        // sent to the data source server
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/properties");
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/properties/" . $id, $this->build_query($options));
        // return as request
        if ($concurrentRequest) {
            return new Request('GET', $url);
        }
