...
class FileMapper extends ATMBaseMapper implements BaseMapper
{
    ...
    public function countByOptions(array $options = [], $concurrentRequest = false): string
    {
        $options[] = 'count';
        // generate the url
        $url = $this->generateUrl("/api/files", $this->build_query($options));
        // return as request
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/files/" . $id);
        if ($concurrentRequest) {
            return new Request('DELETE', $url);
        }
        // sent to the data source server
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/files");
    ...
    {
        // generate the url
        $url = $this->generateUrl("/api/files/" . $id, $this->build_query($options));
        // return as request
        if ($concurrentRequest) {
            return new Request('GET', $url);
        }
