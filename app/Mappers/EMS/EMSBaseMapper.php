...
class EMSBaseMapper
{
    ...
    function __construct()
    {
        ...
        if (Config::get('meowlomo.ems_port') == null) {
            $this->baseUrl = Config::get('meowlomo.ems_protocol') . ":" . Config::get('meowlomo.ems_host');
        } else {
            $this->baseUrl = Config::get('meowlomo.ems_protocol') . "://" . Config::get('meowlomo.ems_host') . ":" . Config::get('meowlomo.ems_port');
        }
    }
    ...
    {   
		...
		if(strcmp(env('CLOUD_CONFIG'),'public')==0){
            ...
            $tenant_id = $tenantInfo->get_tenant_id($this->user->uuid);
