...
class WorkerMapper extends EMSBaseMapper implements MLUuidBaseMapper {

	public function countByOptions(array $options = [], $concurrentRequest = false): string{
		$options[] = 'count';
		// generate the url
		$url = $this->generateUrl("/api/workers", $this->build_query($options));
		// return as request
	...
	public function deleteByUuid($uuid, $concurrentRequest = false): string{
		// generate the url
		$url = $this->generateUrl("/api/workers/" . $uuid);
		if ($concurrentRequest) {
			return new Request('DELETE', $url);
		}
		// sent to the data source server
	...
	public function insert(string $jsonStringBody, $concurrentRequest = false): string{
		// generate the url
		$url = $this->generateUrl("/api/workers");
	...
	public function selectByUuid(array $options = [], $uuid, $concurrentRequest = false): string{
		// generate the url
		$url = $this->generateUrl("/api/workers/" . $uuid, $this->build_query($options));
		// return as request
		if ($concurrentRequest) {
			return new Request('GET', $url);
		}
