...
use App\Services\ATM\RunSetResultService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class RunSetResultController extends MeowlomoBaseController implements MeowlomoBaseControllerInterface
{
    ...
    {
        $this->baseService = new RunSetResultService();
    }
    ...
    // 关联接口
    public function getRunByRunSetResultId(Request $request, $runSetResultId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getRunByRunSetResultId($request, $runSetResultId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function deleteRunByRunSetResultId(Request $request, $runSetResultId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->deleteRunByRunSetResultId($request, $runSetResultId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function insertRunByRunSetResultId(Request $request, $runSetResultId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->insertRunByRunSetResultId($request, $runSetResultId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function replaceRunByRunSetResultId(Request $request, $runSetResultId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->replaceRunByRunSetResultId($request, $runSetResultId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }
    
    public function getRunExecutionInfoByRunSetResultId(Request $request, $runSetResultId) {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        if (!in_array($request->getMethod(), $this->noRequestBodyHttpMethod) && json_last_error() !== JSON_ERROR_NONE) {
        ...
        $jsonStringResponse = $this->baseService->getRunExecutionInfoByRunSetResultId($request, $runSetResultId);
