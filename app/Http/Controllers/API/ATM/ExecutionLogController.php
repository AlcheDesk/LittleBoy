...
use App\Services\ATM\ExecutionLogService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class ExecutionLogController extends MeowlomoBaseController implements MeowlomoBaseControllerInterface
{
    ...
    {
        $this->baseService = new ExecutionLogService();
    }
    ...
    // 关联接口
    public function getInstructionResultByExecutionLogId(Request $request, $executionLogId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getInstructionResultByExecutionLogId($request, $executionLogId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function getRunByExecutionLogId(Request $request, $executionLogId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getRunByExecutionLogId($request, $executionLogId);
