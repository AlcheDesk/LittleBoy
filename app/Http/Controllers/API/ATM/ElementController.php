...
use App\Services\ATM\ElementService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class ElementController extends MeowlomoBaseController implements MeowlomoBaseControllerInterface
{
    ...
    {
        $this->baseService = new ElementService();
    }
    ...
    // 关联接口
    public function getInstructionActionByElementId(Request $request, $elementId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getInstructionActionByElementId($request, $elementId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function getSectionByElementId(Request $request, $elementId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getSectionByElementId($request, $elementId);
