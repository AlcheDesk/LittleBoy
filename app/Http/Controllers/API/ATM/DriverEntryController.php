...
use App\Services\ATM\DriverEntryService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class DriverEntryController extends MeowlomoBaseController implements MeowlomoBaseControllerInterface
{
    ...
    {
        $this->baseService = new DriverEntryService();
    }
    ...
    // 关联接口
    public function getDriverPropertyByDriverEntryId(Request $request, $driverEntryId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getDriverPropertiesByDriverEntryId($request, $driverEntryId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function deleteDriverPropertyByDriverEntryId(Request $request, $driverEntryId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->deleteDriverPropertiesFromDriverEntry($request, $driverEntryId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function insertDriverPropertyByDriverEntryId(Request $request, $driverEntryId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->insertDriverPropertyByDriverEntryId($request, $driverEntryId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function replaceDriverPropertyByDriverEntryId(Request $request, $driverEntryId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->replaceOrInsertDriverPropertyByDriverEntry($request, $driverEntryId);
