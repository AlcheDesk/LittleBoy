...
use App\Services\ATM\InstructionTypeService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class InstructionTypeController extends MeowlomoBaseController implements MeowlomoBaseControllerInterface
{
    ...
    {
        $this->baseService = new InstructionTypeService();
    }
    ...
    // 关联接口
    public function getInstructionByInstructionTypeId(Request $request, $id)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getInstructionByInstructionTypeId($request, $id);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function getElementTypeByInstructionTypeId(Request $request, $id)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getElementTypeByInstructionTypeId($request, $id);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }
    
    public function getInstructionActionByInstructionTypeId(Request $request, $id)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getInstructionActionByInstructionTypeId($request, $id);
