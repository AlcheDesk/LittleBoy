...
use App\Services\ATM\TestCaseOverwriteService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class TestCaseOverwriteController extends MeowlomoBaseController implements MeowlomoBaseControllerInterface
{
    ...
    {
        $this->baseService = new TestCaseOverwriteService();
    }
    ...
    // 关联接口
    public function getInstructionOverwriteByTestCaseOverwriteId(Request $request, $testCaseOverwriteId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getInstructionOverwriteByTestCaseOverwriteId($request, $testCaseOverwriteId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function deleteInstructionOverwriteByTestCaseOverwriteId(Request $request, $testCaseOverwriteId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->deleteInstructionOverwritesFromTestCaseOverwrite($request, $testCaseOverwriteId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function insertInstructionOverwriteByTestCaseOverwriteId(Request $request, $testCaseOverwriteId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->insertInstructionOverwriteByTestCaseOverwriteId($request, $testCaseOverwriteId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function replaceInstructionOverwriteByTestCaseOverwriteId(Request $request, $testCaseOverwriteId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->replaceOrInsertInstructionOverwriteByTestCaseOverwrite($request, $testCaseOverwriteId);
