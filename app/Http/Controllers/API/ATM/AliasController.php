...
use App\Services\ATM\AliasService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use App\Http\Controllers\API\MeowlomoBaseController;

class AliasController extends MeowlomoBaseController implements MeowlomoBaseControllerInterface
{
    ...
    {
        $this->baseService = new AliasService();
    }
    ...
    // 关联接口
    public function getRunSetByAliasId(Request $request, $aliasId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->getRunSetByAliasId($request, $aliasId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function deleteRunSetByAliasId(Request $request, $aliasId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->deleteRunSetByAliasId($request, $aliasId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function insertRunSetByAliasId(Request $request, $aliasId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->insertRunSetByAliasId($request, $aliasId);

        Log::debug('Controller layer ' . __CLASS__ . ' ' . __FUNCTION__ . ' return ' . var_export($jsonStringResponse, true));
        return response($jsonStringResponse, 200)->header('Content-Type', 'application/json; charset=utf8');
    }

    public function replaceRunSetByAliasId(Request $request, $aliasId)
    {
        $bodyContent = $request->getContent();
        // check the body is json
        json_decode($bodyContent);
        ...
        $jsonStringResponse = $this->baseService->replaceRunSetByAliasId($request, $aliasId);
