...
use App\Services\EMS\JobService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class JobController extends MeowlomoBaseController implements MeowlomoUuidBaseControllerInterface {

	private $baseService;

	public function __construct() {
		$this->baseService = new JobService();
	}
	...
	// 关联接口
	public function getTaskByJobUuid(Request $request, $jobUuid) {
		$bodyContent = $request->getContent();
		// check the body is json
		json_decode($bodyContent);
		if (!in_array($request->getMethod(), $this->noRequestBodyHttpMethod) && json_last_error() !== JSON_ERROR_NONE) {
		...
		$jsonStringResponse = $this->baseService->getTaskByJobUuidRequest($request, $jobUuid);
